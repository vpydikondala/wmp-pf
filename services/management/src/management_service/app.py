...
@app.delete(
    "/api/v1/management/buildings/{item_id}",
    status_code=204,
)
def delete_building(item_id: UUID):
    return _delete("buildings", item_id)
...
def list_floors(building_id: UUID | None = None):
    return _rows(
        ...
        SELECT *
        FROM floors
        WHERE (%s::uuid IS NULL OR building_id=%s)
        ORDER BY floor_number NULLS LAST, name
        """,
...
@app.delete(
    "/api/v1/management/floors/{item_id}",
    status_code=204,
)
def delete_floor(item_id: UUID):
    return _delete("floors", item_id)
...
def list_zones(floor_id: UUID | None = None):
    return _rows(
        ...
        SELECT *
        FROM zones
        WHERE (%s::uuid IS NULL OR floor_id=%s)
        ORDER BY name
        """,
        (floor_id, floor_id),
    )
...
@app.delete(
    "/api/v1/management/zones/{item_id}",
    status_code=204,
)
def delete_zone(item_id: UUID):
    return _delete("zones", item_id)
...
def list_access_points(floor_id: UUID | None = None):
    return _rows(
        ...
        FROM access_points
        WHERE (%s::uuid IS NULL OR floor_id=%s)
        ORDER BY name NULLS LAST, serial
        """,
        (floor_id, floor_id),
    )
...
@app.post(
    "/api/v1/management/access-points",
    status_code=201,
)
...
@app.delete(
    "/api/v1/management/access-points/{item_id}",
    status_code=204,
)
def delete_access_point(item_id: UUID):
    return _delete("access_points", item_id)
...
@app.delete(
    "/api/v1/management/desks/{item_id}",
    status_code=204,
)
def delete_desk(item_id: UUID):
    return _delete("desks", item_id)
...
def floor_layout(floor_id: UUID):
    ...
    zones = _rows(
        "SELECT * FROM zones WHERE floor_id=%s ORDER BY name",
        (floor_id,),
    )

    desks = _rows(
        "SELECT * FROM desks WHERE floor_id=%s ORDER BY name",
        (floor_id,),
    )
