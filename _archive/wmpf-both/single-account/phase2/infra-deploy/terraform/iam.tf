...
data "aws_iam_policy_document" "ecs_task" {
  ...
  dynamic "statement" {
    for_each = var.deployment.data_bucket && contains(["inbound", "dashboard_sync", "processor"], each.key) ? [1] : []
    content {
      sid = "DataBucket"
      actions = [
        "s3:GetObject",
  ...
  dynamic "statement" {
    for_each = var.deployment.processing_queue && contains(["inbound", "dashboard_sync", "processor"], each.key) ? [1] : []
    content {
      sid = "ProcessingQueue"
      actions = [
        "sqs:SendMessage",
  ...
  dynamic "statement" {
    for_each = var.deployment.secrets_manager && contains(["inbound", "dashboard_sync"], each.key) ? [1] : []
    content {
      sid       = "MerakiSecret"
      actions   = ["secretsmanager:GetSecretValue"]
...
resource "aws_iam_role" "dashboard_sync_events" {
  name = "${local.name_prefix}-dashboard-sync-events"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{ Effect = "Allow", Principal = { Service = "events.amazonaws.com" }, Action = "sts:AssumeRole" }]
  })
}
resource "aws_iam_role_policy" "dashboard_sync_events" {
  name = "${local.name_prefix}-dashboard-sync-events"
  role = aws_iam_role.dashboard_sync_events.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      { Effect = "Allow", Action = ["ecs:RunTask"], Resource = "*" },
      { Effect = "Allow", Action = ["iam:PassRole"], Resource = concat(
